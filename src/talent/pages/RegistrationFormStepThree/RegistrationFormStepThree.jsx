...
export default function RegistrationFormStepThree() {
  ...
  // Ish joyi turlari uchun massiv (Tarjima bilan)
  const WORKPLACE_OPTIONS = [
    { id: "Onsite", label: t("step3.workplace_options.onsite"), icon: <FaBuilding /> },
    { id: "Remote", label: t("step3.workplace_options.remote"), icon: <FaHome /> },
    { id: "Hybrid", label: t("step3.workplace_options.hybrid"), icon: <FaFlag /> },
  ];

  // Bandlik turlari (TYPES) tarjimasi
  const TYPES = [
    { id: "fulltime", label: t("step3.types.fulltime"), icon: <FaBriefcase /> },
    { id: "parttime", label: t("step3.types.parttime"), icon: <FaClock /> },
    { id: "contract", label: t("step3.types.contract"), icon: <FaFileContract /> },
    { id: "freelance", label: t("step3.types.freelance"), icon: <FaUserTie /> },
  ];
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      setWorkplaceType(parsed.workplaceType || "Onsite");
  ...
  }, [workplaceType, minimumSalary, city, errors.workplaceType, errors.minimumSalary, errors.city]);
  ...
  const handleFinish = async () => {
    ...
    if (newErrors.workplaceType || newErrors.minimumSalary || newErrors.city) {
      toast.error(t("step3.error_fields"));
      return;
    }
    ...
    try {
      ...
      if (!allData.email) {
        toast.error(t("step3.error_email"));
        setLoading(false);
        return;
      }
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        toast.success(t("step3.success_msg"));
  ...
  const idx = Math.max(0, TYPES.findIndex((t) => t.id === employmentType));
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-5 xs:p-6 sm:p-8 md:p-10 lg:p-12">
            ...
            <div className="space-y-5 sm:space-y-6 md:space-y-8">
                ...
                <div className="relative bg-gray-100 rounded-xl sm:rounded-[40px] border border-gray-200 grid grid-cols-2 sm:grid-cols-4 p-0.5 overflow-hidden">
                  ...
                  {TYPES.map((type) => (
                    <button
                      ...
                      className={`relative z-10 min-w-0 flex items-center sm:flex-col justify-center gap-1 sm:gap-0.5 py-1.5 sm:py-2 md:py-2.5 font-medium transition-colors duration-200 ${employmentType === type.id ? "text-[#163D5C]" : "text-gray-400"
                        }`}
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 xs:gap-5 sm:gap-6">
                {/* Workplace Type Slider */}
                <div>
                  <label className="block text-gray-700 font-medium mb-2 text-sm xs:text-base">
                    {t("step3.workplace_type")} *
                  </label>
                  <div
                    className={`relative bg-gray-100 rounded-[50px] border flex overflow-hidden ${errors.workplaceType ? "border-red-500" : "border-gray-200"
                      }`}
                  >
                    <div
                      className="absolute top-0 left-0 h-full w-1/3 bg-white rounded-[50px] shadow-md transition-all duration-300"
                      style={{
                        transform: `translateX(${WORKPLACE_OPTIONS.findIndex((opt) => opt.id === workplaceType) * 100}%)`,
                      }}
                    ></div>
                    {WORKPLACE_OPTIONS.map((option) => (
                      <button
                        key={option.id}
                        type="button"
                        onClick={() => setWorkplaceType(option.id)}
                        className={`flex-1 flex items-center justify-center gap-1 xs:gap-2 py-2.5 xs:py-3 sm:py-3.5 relative z-10 font-semibold transition-colors duration-200 text-xs xs:text-sm sm:text-base ${workplaceType === option.id ? "text-[#163D5C]" : "text-gray-400"
                          }`}
                      >
                        <span className="text-xs xs:text-sm sm:text-base">{option.icon}</span>
                        {option.label}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <FaDollarSign
                      className={`absolute left-3 xs:left-4 top-1/2 -translate-y-1/2 text-sm xs:text-base ${errors.minimumSalary ? "text-red-500" : "text-[#163D5C]"
                        }`}
                    />
                    <input
                      type="number"
                      ...
                      className={`w-full pl-9 xs:pl-10 sm:pl-12 pr-3 xs:pr-4 py-2.5 xs:py-3 border rounded-lg outline-none text-sm xs:text-base ${errors.minimumSalary
                          ? "border-red-500 bg-red-50"
                          : "border-gray-200 focus:ring-2 focus:ring-[#163D5C]"
                        }`}
                      placeholder={t("step3.placeholder_salary")}
                      min="0"
                    />
                  </div>
                  {errors.minimumSalary && (
                    <p className="text-red-500 text-xs mt-1">{t("step3.error_salary_msg")}</p>
                  )}
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 xs:gap-5 sm:gap-6">
                {/* City Selection - Agar bu text input bo'lsa tarjima qilish */}
                <div>
                  <label className="block text-gray-700 font-medium mb-2 text-sm xs:text-base">
                    {t("step3.city")} *
                  </label>
                  <div className="relative">
                    <FaCity
                      className={`absolute left-3 xs:left-4 top-1/2 -translate-y-1/2 text-sm xs:text-base ${errors.city ? "text-red-500" : "text-[#163D5C]"
                        }`}
                    />
                    <input
                      type="text"
                      value={city.includes("cities.") ? t(`step3.${city}`) : city}
                      onChange={(e) => setCity(e.target.value)}
                      className={`w-full pl-9 xs:pl-10 sm:pl-12 pr-3 xs:pr-4 py-2.5 xs:py-3 border rounded-lg outline-none text-sm xs:text-base ${errors.city
                          ? "border-red-500 bg-red-50"
                          : "border-gray-200 focus:ring-2 focus:ring-[#163D5C]"
                        }`}
                      placeholder={t("step3.placeholder_city")}
                    />
                  </div>
                  {errors.city && (
                    <p className="text-red-500 text-xs mt-1">{t("step3.error_city_msg")}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t("step3.placeholder_country")}
                    />
                  </div>
                </div>
              </div>
