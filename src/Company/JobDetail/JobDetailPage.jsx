  ...
  <div className="max-w-5xl mx-auto animate-pulse">
    ...
    <div className="space-y-6">
      {[1, 2].map((i) => (
        <div
          key={i}
          className="h-44 bg-white border border-gray-100 rounded-[2rem]"
        ></div>
      ))}
    </div>
  </div>
...
const JobDetailPageCompany = () => {
  ...
  const handleConfirmInvitation = async () => {
    ...
    try {
      // 1. Notification yuborish (bu qism ishlayapti)
      ...
      // 2. Application (taklifnoma) yaratish
      // Sizda 'apply' funksiyasi FormData kutadi, shuning uchun uni tayyorlaymiz:
      const data = new FormData();
      data.append("job_id", Number(id));
      data.append("talent_id", Number(selectedTalent.id));
      data.append("status", "pending");

      // applicationApi.create emas, aynan .apply ni chaqiramiz
      ...
      <div className="max-w-5xl mx-auto relative">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full mt-1 px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">
                  ...
                  <textarea
                    ...
                    className="w-full mt-1 px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                  />
                </div>
        ...
        {isEditModalOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="bg-white w-full max-w-2xl rounded-[2.5rem] p-6 sm:p-10 shadow-2xl relative max-h-[90vh] overflow-y-auto text-left">
              ...
              <h2 className="text-2xl font-bold text-[#343C44] mb-8">
                Edit Job Position
              </h2>
              <form
              ...
              >
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-[#F8F9FA] border border-gray-100 rounded-xl"
                    required
                  />
                </div>
                <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 bg-[#F8F9FA] border border-gray-100 rounded-xl resize-none"
                  />
                </div>
                <button
                  type="submit"
                  className="md:col-span-2 w-full mt-4 py-4 bg-[#5CB85C] text-white font-bold rounded-2xl"
                >
                  Update Job Details
                </button>
              </form>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 mt-2 md:mt-0 w-full">
          ...
          <button
            ...
            className="bg-[#5CB85C] hover:bg-[#4cae4c] text-white w-full md:w-auto px-10 py-4 rounded-2xl font-bold"
          >
            Post a Job
          </button>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-6 sm:p-10 shadow-sm border border-gray-100 mb-10 relative">
          <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-8 mb-8 lg:pr-12">
            <div className="flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left">
              ...
              <div className="max-w-md">
                <h2 className="text-2xl sm:text-3xl font-bold text-[#343C44] leading-tight">
                  {job.occupation}
                </h2>
                <p className="text-[#8E8E8E] font-[600] text-[22px] mt-1">
          ...
          <p className="text-[#343C44] text-base sm:text-lg leading-relaxed mb-8 text-center md:text-left">
            {job.description}
          </p>
          <div className="pt-6 border-t border-gray-50 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="w-full">
              <h4 className="text-[#343C44] font-bold mb-4 uppercase text-xs sm:text-sm tracking-widest text-center md:text-left">
                Required skills
              </h4>
              <div className="flex flex-wrap justify-center md:justify-start gap-2 sm:gap-3">
                {getJobSkills().map((skill, index) => (
                  <span
                    key={index}
                    className="px-4 sm:px-5 py-1.5 sm:py-2 bg-[#F3F4F6] text-[#4B5563] rounded-xl font-semibold border border-gray-100 text-sm sm:text-base"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="flex items-center gap-3 shrink-0">
              <button
                ...
                className="p-4 bg-gray-50 text-gray-400 rounded-2xl hover:bg-[#73dd73] hover:text-white border border-gray-100"
              >
                <FiEdit2 size={22} />
              </button>
              <button
                onClick={() => setIsDeleteModalOpen(true)}
                className="p-4 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white border border-red-100"
              >
                <FiTrash2 size={22} />
              </button>
            </div>
        ...
        <button
          ...
          className="group flex items-center justify-center gap-3 text-[#1D3D54] font-bold mb-10 w-full sm:w-[240px] h-[60px] border-2 border-[#1D3D54] rounded-2xl bg-white shadow-sm transition-all duration-300 hover:bg-[#1D3D54] hover:text-white"
        >
          <MdKeyboardArrowLeft
            size={28}
        ...
        <div className="flex bg-[#E9E9E9] p-1.5 rounded-[1.5rem] w-full relative mb-10 overflow-hidden">
          <div
            className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white rounded-[1.2rem] shadow-sm transition-all duration-300 ${activeDetailTab === "Matches" ? "left-1.5" : "left-[calc(50%+3px)]"}`}
          />
          <button
        ...
        <div className="pb-20">
          ...
          <div className="grid grid-cols-1 min-[1350px]:grid-cols-2 gap-6 sm:gap-8 w-full">
            ...
            {(activeDetailTab === "Matches"
              ...
              return (
                talent && (
                  <div
                    key={uniqueKey}
                    className="bg-white border border-gray-100 rounded-[2rem] shadow-sm p-6 sm:p-7 flex flex-col justify-between hover:shadow-md transition-all min-h-[400px]"
                  >
                    <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-5 mb-5 w-full">
                      <div className="flex flex-col lg:flex-row items-center gap-4 flex-1 w-full min-w-0">
                        ...
                        <div className="w-full min-w-0 text-center lg:text-left">
                          <h2 className="text-[20px] sm:text-[24px] font-bold text-[#3a3a3a] leading-tight mb-1">
                      ...
                      <div className="shrink-0 w-full lg:w-auto border-t lg:border-none pt-4 lg:pt-0 text-center lg:text-right">
                        <div className="text-[22px] sm:text-[26px] font-bold text-[#1D3D54] leading-tight">
                      ...
                      <div className="flex flex-wrap justify-center lg:justify-start gap-2">
                        {parseTalentSkills(talent.skils)
                          ...
                          .map((s, i) => (
                            <span
                              key={i}
                              className="px-4 py-1.5 bg-[#f8fafc] text-[#64748b] text-[13px] font-semibold rounded-xl border border-gray-100"
                            >
                              {s.skill}
                            </span>
                          ))}
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-auto items-center justify-center w-full">
                      ...
                      {activeDetailTab === "Matches" ? (
                        <button
                          ...
                          className="flex-1 py-3 bg-[#1D3D54] text-white font-bold rounded-xl flex items-center justify-center gap-2"
                        >
                          <FiSend /> Invite
                        </button>
                      ) : (
                        <div className="flex-1 w-full flex items-center justify-center gap-3">
                          {status === "pending" ? (
                            ...
                            <div
                              className={`flex-1 py-3 text-center rounded-2xl font-bold uppercase text-xs tracking-widest border-2 ${status === "accepted" ? "border-emerald-500 bg-emerald-50 text-emerald-600" : "border-rose-500 bg-rose-50 text-rose-600"}`}
                            >
                              {status}
                            </div>
                          )}
            ...
            {activeDetailTab === "Invitations" && applications.length === 0 && (
              <div className="col-span-full text-center py-20 bg-white rounded-[2rem] border border-dashed border-gray-200 text-gray-400">
